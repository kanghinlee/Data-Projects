...
g7_prod %>% 
  ...
  geom_point()+
  geom_line()+
  scale_linetype_manual(values = c("g7_avg" = "dotted"))+
  labs(title = "Productivity in G7 Countries",
       )+
  xlab("Year")+
  ylab("Output Per Hour Worked (£)")+
  theme(axis.text.x = element_text(angle = 45))
  # scale_color_manual(values = c("us" = "blue", "uk" = "red"))+
```
# G7 - GDP
```{r}
...
g7_gdp_avg <- g7_gdp %>% 
  group_by(year) %>% 
  summarise(g7_avg = mean(output_per_hour_worked))

g7_gdp %>% 
  ggplot()+
  aes(x = year, y = output_per_hour_worked, group = country, colour = country)+
  geom_point()+
  geom_line()+
  geom_line(aes(x = g7_gdp_avg$year, y = g7_gdp_avg$g7_avg, type = 2 ))+
  scale_color_manual(values = c("us" = "blue", "uk" = "red"))+
  labs(title = "GDP in G7 Countries",
       )+
  xlab("Year")+
  ylab("GDP (in million £)")+
  theme(axis.text.x = element_text(angle = 45))
